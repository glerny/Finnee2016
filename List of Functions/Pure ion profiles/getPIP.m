function LstPIP = getPIP(LoPts, ThrMZ, ThrBk, ThrIt, minPts, axe, infoDts)
...
for ii = 1:length(FstCut)-1
    ...
    LFC    = sortrows(LFC, 3);
    SndCut = [1; find(diff(LFC(:,3)) > ThrBk+1)+1; size(LFC,1)+1];
    for jj = 1:length(SndCut) -1
        ...
        if size(cPIP, 1) >= minPts && max(cPIP(:,2)) >= ThrIt
            ...
            if ~isempty(ttd)
                LstPIP = [LstPIP, getPIP(cPIP, ThrMZ, ThrBk, ThrIt, minPts, axe, infoDts)];
            else
