function new_HyperROIs = addROIstoHyperROIs(old_HyperROIs, TAG, files)
...
% 1. Introduction
dts   = HyperROIs.parameters.dts = dts;
MzWdW = HyperROIs.parameters.MzWdW = MzWdW;
timeWdw = HyperROIs.parameters.timeWdw = timeWdw;
HyperROIs.parameters.master = fullfile(obj.Path, obj.Name);
HyperROIs.parameters.QCFiles = obj.QC.Files;
...
HyperROIs.parameters.Path = fullfile(obj.Path, nameDir);
HyperROIs.Ids = {};
HyperROIs.Tags = [];
HyperROIs.ListOfTags = {'QC1'};

% 2. Setting Limits for ROIs
% 2.1. Open first QC
MF = load(fullfile(HyperROIs.parameters.QCFiles{1}, 'myFinnee.mat'));
myFinnee = MF.myFinnee;
HyperROIs.Ids{1} = myFinnee.FileID;
HyperROIs.Tags{1} = 'QC1';
...
for cF = 2:length(HyperROIs.parameters.QCFiles)
    MF = load(fullfile(HyperROIs.parameters.QCFiles{cF}, 'myFinnee.mat'));
    myFinnee = MF.myFinnee;
    HyperROIs.Ids{cF} = myFinnee.FileID;
    HyperROIs.Tags{cF} = 'QC1';
    [ROI, X, Y] = myFinnee.Datasets{dts}.mkMnROI_2( FOMs.mz_min,  ...
        FOMs.mz_max, FOMs.Time_min, FOMs.Time_max);
...
for ii = 1:size(HyperROIs.Data, 2)
    ...
    allProf = doAlignment_MinPearson(allProf, 30);
...
file = fullfile(obj.Path, nameDir, 'HyperROIs.mat');
save(file, 'HyperROIs');
