...
function dtsOut = doBslCor_old(dtsIn, par4bas, path2Dir, MaxFileSize, m)
...
fidWrite = fopen(Link2CorPrf, 'ab');

h = waitbar(0,'Correcting profiles');
for ii = 1:nbrProf
    waitbar(ii/nbrProf)
    fseek(fidRead, (ii-1)*4, 'bof');
    Prf = fread(fidRead, lengthTm, par4bas.obj.Precision, (nbrProf-1)*4);
    ...
    switch par4bas.type
        case 'None'
            ...
            indNeg = yy <= 0;
            yy(indNeg) = 0;
            fwrite(fidWrite, yy, par4bas.obj.Precision);
...
fidRead = fopen(Link2CorPrf, 'rb');
for ii = 1:2*wdz+1
    XMS = dtsIn.xpend(dtsIn.ListOfScans{ii});
    fseek(fidRead, (ii-1)*4, 'bof');
    Corr = fread(fidRead, nbrProf, par4bas.obj.Precision, (lengthTm-1)*4);
