...
function TIS = multiTIS(obj, Limits, k)
...
for ii = 1:size(Limits, 1)
    ...
    TIS{ii, 1}(:,1) = AxisY(indMZ(ii, 1):indMZ(ii, 2));
    TIS{ii, 1}(:,2) = 0;
end
...
switch obj.Format
    case 'profile'
        if ~isempty(obj.AxisY.Data)
            ...
            h = waitbar(0,'Bulding Total Ion Spectra, please wait');
            for ii = IdTLim(1):IdTLim(2)
                ...
                for jj = 1:length(id)
                    if nnz(XMS.Data(indMZ(id(jj),1):indMZ(id(jj),2), 2)) >= k
                        try
                        TIS{id(jj)}(:,2) = TIS{id(jj)}(:,2) + XMS.Data(indMZ(id(jj),1):indMZ(id(jj),2), 2);
                        catch
                        disp("pp")
                        end
                    end
